...
class Memory:
    ...
    def record(self, state, channel_action, power_action, reward, done,
               log_prob, v_state, target_ap_idx):
    ...
    def compute_returns(self, gamma=0.99, normalize_rewards=1.0):
        """
        Compute discounted returns using standard method.
        ...
        for t in reversed(range(len(norm_reward_mb))):
            future_return = norm_reward_mb[t] + gamma * future_return
...
class PPO:
    ...
    def train_actor(self, bufferdata: RolloutBuffer, batch_size: int = 64,
                    ...
                    grad_control: bool = True) -> tuple:
        """
        Train actor using PPO objective.

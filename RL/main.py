...
class Config:
    ...
    CHANNEL_WIDTH_OPTIONS = [80, 40, 80]
    ...
    warmup_steps = 1    # Initial exploration before critic updates
    warmup_critic = 0      # Critic warmstart updates,(this*window_steps+total_critic_warmup_step))
    ...
    n_epochs = 2 # How many times to iterate over the buffer per update

    # Kafka - use environment variables if available, otherwise defaults
    kafka_broker = os.environ.get('KAFKA_BROKER', 'localhost:9092')
    metrics_topic = os.environ.get('METRICS_TOPIC', 'ns3-metrics')
    commands_topic = os.environ.get('COMMANDS_TOPIC', 'optimization-commands')
    rl_events_topic = os.environ.get('RL_EVENTS_TOPIC', 'rl-events')
    simulation_id = os.environ.get('SIMULATION_ID', 'basic-sim')  # Must match NS3 simulation ID!

    # Timing
    action_cooldown = 5.0   # Seconds between actions
    config_settling_time = 1.0  # Seconds to wait after sending config for NS3 to apply it

    # GAT State Provider settings or HOw fast we call ppo to take actions
    gat_batch_size = 30 # Number of logs to EWMA before taking action (reduced from 180 for testing)
...
class KafkaInterface:
    ...
    def send_action(self, ap_bssid: str, channel_number: int, channel_width: int,
        ...
        def get_colour(channel: int) -> int:
            """Find color ID (0-5) for a given channel number."""
            for color_id, width_map in self.config.COLOR_CHANNEL_MAP.items():
                for width, channels in width_map.items():
                    if channel in channels:
                        return color_id
            return 0  # Default to color 0 if not found
    ...
    def publish_objective_event(
        self,
        sim_time_start: Optional[float],
        sim_time_end: Optional[float],
        step: int,
        ...
        """
        Publish objective calculation event to rl-events topic.

        Args:
            sim_time_start: Simulation time at start of batch
            sim_time_end: Simulation time at end of batch
            step: Training step number
            global_objective: Global objective value (0-1)
            baseline_objective: Baseline objective at t=1
            best_objective: Best objective seen so far
            delta_reward: Change from previous objective
            pct_improvement: Percentage improvement vs baseline
            num_aps: Number of APs in network
            per_ap_objectives: Per-AP objective breakdown
            avg_metrics: Averaged metrics across APs

        Returns:
            True if published successfully, False otherwise
        """
        if not self._connected:
            print(f"[MOCK] Objective event: global={global_objective:.4f}, "
                  f"delta={delta_reward:.4f}, num_aps={num_aps}")
            return True
        ...
        try:
            message = {
                ...
                'num_aps': num_aps,
                'per_ap_objectives': per_ap_objectives,
                'avg_metrics': avg_metrics
            }

            future = self.producer.send(
                self.config.rl_events_topic,
                key=self.config.simulation_id.encode('utf-8'),
                value=message
            )
            future.get(timeout=10)
            ...
            print(f"[KAFKA RL-EVENTS ERROR] Failed to publish objective event: {e}")
            return False
    ...
    def publish_config_event(
        ...
        sim_time_start: Optional[float],
        sim_time_end: Optional[float],
        effect_at_sim_time: Optional[float],
        step: int,
        target_ap_bssid: str,
        ...
        """
        Publish configuration sent event to rl-events topic.

        Args:
            sim_time_start: Simulation time at start of batch
            sim_time_end: Simulation time at end of batch
            effect_at_sim_time: Simulation time when config takes effect
            step: Training step number
            target_ap_bssid: BSSID of target AP
            selection_reason: Why this AP was selected
            graph_color: Graph coloring color ID (0-5)
            channel_number: Actual channel number
            channel_width: Channel width in MHz
            width_action_idx: PPO width action index
            power_dbm: Transmit power in dBm
            power_action_idx: PPO power action index
            action_source: "RANDOM (warmup)" or "PPO Policy"
            log_prob: Log probability of action
            warmup_phase: Whether in warmup phase
            critic_warmstart_phase: Whether in critic warmstart phase
            pre_action_state: AP state before action

        Returns:
            True if published successfully, False otherwise
        """
        if not self._connected:
            print(f"[MOCK] Config event: AP={target_ap_bssid}, "
                  f"channel={channel_number}, width={channel_width}MHz, "
        ...
        try:
            message = {
                ...
                'sim_time_start': sim_time_start,
                'sim_time_end': sim_time_end,
                'effect_at_sim_time': effect_at_sim_time,
                'simulation_id': self.config.simulation_id,
                'step': step,
                'target_ap': {
            ...
            print(f"[KAFKA RL-EVENTS ERROR] Failed to publish config event: {e}")
            return False
...
def main():
    ...
    # Initialize GAT State Provider (direct Kafka consumption, with file fallback)
    gat_provider = GATStateProvider(
        broker=config.kafka_broker,
        topic=config.metrics_topic,
        simulation_id=config.simulation_id,
        use_kafka=True,  # Set to False to use file mode for local testing
    ...
    if not gat_provider.connect():
        print("ERROR: Failed to connect GATStateProvider")
        return
    ...
    selected_aps_in_cycle = set()  # Track which APs have been selected in current cycle
    ...
    # Create structured JSON logger for publishable logs
    rl_logger = RLLogger(output_dir=str(output_dir), verbose=config.verbose)
    logger.info(f"Structured JSON logging to: {rl_logger.get_log_file_path()}")
    ...
    try:
        while update_count < config.max_updates:
            ...
            # 3. Select AP using worst-first round-robin
            # If all APs have been selected in this cycle, reset the cycle
            if len(selected_aps_in_cycle) >= state.num_aps:
                selected_aps_in_cycle.clear()
                if config.verbose:
                    logger.info(f"[AP SELECTION] Cycle complete, resetting for new cycle")

            # Compute per-AP objectives for all APs
            per_ap_objectives = compute_all_per_ap_objectives(gat_state, verbose=False)

            # Filter out already-selected APs from consideration
            ...
            if remaining_objectives:
                # Find the worst (lowest objective) AP from remaining APs
                worst_ap_bssid = min(remaining_objectives, key=remaining_objectives.get)
                target_ap_bssid = worst_ap_bssid
                target_ap_idx = state.ap_bssids.index(target_ap_bssid)
                selection_reason = "worst_first_round_robin"

                # Mark this AP as selected in current cycle
                selected_aps_in_cycle.add(target_ap_bssid)

                if config.verbose:
                    logger.info(f"[AP SELECTION] Selected worst AP from remaining: {target_ap_bssid} "
                              f"(objective: {remaining_objectives[worst_ap_bssid]:.4f}). "
                              f"Cycle progress: {len(selected_aps_in_cycle)}/{state.num_aps}")
            else:
                # Fallback: if no valid objectives, use simple round-robin
                target_ap_idx = len(selected_aps_in_cycle) % state.num_aps
                target_ap_bssid = state.ap_bssids[target_ap_idx]
                selection_reason = "fallback_round_robin"
                selected_aps_in_cycle.add(target_ap_bssid)
                if config.verbose:
                    logger.info(f"[AP SELECTION] Fallback: selected AP {target_ap_bssid} (no valid objectives)")
            ...
            if config.verbose:
                ...
                for bssid in state.ap_bssids:
                    ...
                    if comp is not None:
                        marker = " <-- SELECTED" if bssid == target_ap_bssid and (selection_reason == "worst_first_round_robin" or selection_reason == "fallback_round_robin") else ""
            ...
            # Determine width based on TOTAL number of clients across ALL APs
            total_clients = sum(
                int(node_data.get('num_clients', 0))
                for node_data in gat_state.get('nodes', {}).values()
            )
            ...
            # Map to channel number using color and selected width
            channel_options = config.COLOR_CHANNEL_MAP[target_color][width_val]
            channel_number = random.choice(channel_options)
            ...
            if config.verbose:
                ...
                logger.info(f"  - Width: {width_val} MHz")
            ...
            if send_success:
                ...
                # Publish config event to rl-events topic
                sim_time_range = gat_state.get('sim_time_range', {})
                target_node_data = gat_state.get('nodes', {}).get(target_ap_bssid, {})
                pre_action_state = {
                    'channel': int(target_node_data.get('channel', 0)),
                    'power': float(target_node_data.get('power', 0)),
                    'num_clients': int(target_node_data.get('num_clients', 0)),
                    'channel_utilization': float(target_node_data.get('channel_utilization', 0))
                }
                kafka.publish_config_event(
                    sim_time_start=sim_time_range.get('start'),
                    sim_time_end=sim_time_range.get('end'),
                    effect_at_sim_time=sim_time_range.get('effect_at'),
                    step=step,
                    target_ap_bssid=target_ap_bssid,
                    selection_reason=selection_reason,
                    graph_color=target_color,
                    channel_number=channel_number,
                    channel_width=width_val,
                    width_action_idx=width_action,
                    power_dbm=config.POWER_OPTIONS[power_action],
                    power_action_idx=power_action,
                    action_source=action_source,
                    log_prob=log_prob,
                    warmup_phase=step < config.warmup_steps,
                    critic_warmstart_phase=in_critic_warmstart,
                    pre_action_state=pre_action_state
                )
            else:
            ...
            # Compute % improvement vs baseline for logging
            if step == 0:
                pct_improvement_for_log = None
            elif curr_objective is not None and 'baseline_objective_t1' in dir() and baseline_objective_t1 != 0:
                pct_improvement_for_log = ((curr_objective - baseline_objective_t1) / baseline_objective_t1 * 100)
            else:
                pct_improvement_for_log = None

            # Build per-AP objectives with components for structured logging
            per_ap_objectives_with_components = {}
            for bssid in state.ap_bssids:
                comp = compute_per_ap_objective_with_components(gat_state, bssid)
                if comp is not None:
                    per_ap_objectives_with_components[bssid] = comp

            # Publish objective event to rl-events topic
            sim_time_range = gat_state.get('sim_time_range', {})
            # Compute avg_metrics from per_ap_objectives_with_components
            if per_ap_objectives_with_components:
                avg_metrics = {
                    'throughput_mbps': np.mean([v['p50_throughput'] for v in per_ap_objectives_with_components.values()]),
                    'loss_rate_pct': np.mean([v['p75_loss_rate'] for v in per_ap_objectives_with_components.values()]),
                    'rtt_ms': np.mean([v['p75_rtt'] for v in per_ap_objectives_with_components.values()]),
                    'jitter_ms': np.mean([v['p75_jitter'] for v in per_ap_objectives_with_components.values()])
                }
            else:
                avg_metrics = {'throughput_mbps': 0.0, 'loss_rate_pct': 0.0, 'rtt_ms': 0.0, 'jitter_ms': 0.0}

            kafka.publish_objective_event(
                sim_time_start=sim_time_range.get('start'),
                sim_time_end=sim_time_range.get('end'),
                step=step,
                global_objective=curr_objective,
                baseline_objective=baseline_objective_t1 if 'baseline_objective_t1' in dir() else None,
                best_objective=config.best_objective,
                delta_reward=reward,
                pct_improvement=pct_improvement_for_log,
                num_aps=state.num_aps,
                per_ap_objectives=per_ap_objectives_with_components,
                avg_metrics=avg_metrics
            )

            # Log structured step data (JSONL format for analysis/publishing)
            rl_logger.log_step(
                step=step,
                ...
                power_dbm=config.POWER_OPTIONS[power_action],
                power_action_idx=power_action,
                target_ap_color=target_color,
                log_prob=log_prob,
                action_source=action_source,
                current_objective=curr_objective,
                previous_objective=prev_objective if step > 0 else None,
                delta_reward=reward,
                baseline_objective=baseline_objective_t1 if 'baseline_objective_t1' in dir() else None,
                pct_improvement=pct_improvement_for_log,
                best_objective=config.best_objective,
            ...
            if step >= config.warmup_steps and (step - config.warmup_steps) % config.window_steps == 0:
                ...
                if len(returns) > 0:
                    try:
                        ...
                        try:
                            ppo.save(
                                './online_weights/wifi_actor_new.pth',
                                './online_weights/wifi_critic_new.pth'
                            )
        ...
        # Cleanup
        kafka.close()
        gat_provider.close()  # Close GAT provider (Kafka consumer if in Kafka mode)
        rl_logger.close()  # Close structured JSON logger

        # Save final model (same files as periodic saves)
        ppo.save('./online_weights/wifi_actor_new.pth', './online_weights/wifi_critic_new.pth')

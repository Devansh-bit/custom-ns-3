                  ...
                  Ptr<UniformRandomVariable> uniformRv, double wifiPsd)
{
    ...
    for (int i = 0; i < duration/0.1; i++)
    {
        int channelIdx = uniformRv->GetInteger(0, 10);
        double frequency = wifiChannels[channelIdx];
        double duty_ratio = uniformRv->GetValue(0.1, 0.5);
        double duty_period = uniformRv->GetValue(0.01, 0.03);
        helper.GenerateDutyWifiInterferer(channel, node, frequency - 10e6, startTime + i*0.1, 0.1, duty_period, duty_ratio, 20e6, wifiPsd);
    }
}
                       ...
                       double btPsd)
{
  ...
  //       helper.GenerateBluetoothInterferer(channel, node, periodStart, hops,
  //                                         625e-6, 1e-6, btPsd, hoppingSeed,
  //                                         uniformRv, jitterRv);
  //     }
  //   }
  // }
                    ...
                    Ptr<UniformRandomVariable> uniformRv, double zigbeePsd)
{
  ...
  std::vector<int> usedChannels;
    int numChannelHops = duration / 0.1;
  ...
  for (int hop = 0; hop < numChannelHops; hop++)
  {
    ...
    usedChannels.push_back(zigbeeChannel);
     ...
     helper.GenerateZigbeeInterferer(channel, node, zigbeeChannel, startTime + hop * 0.1, 0.1, 5e-6, uniformRv);
  }
}
                       ...
                       Ptr<UniformRandomVariable> uniformRv, double microwavePsd)
{
  ...
  helper.GenerateDutyMicrowaveInterferer(channel, node, centerFreq, startTime,
                                    duration, acPeriod, dutyCycle, bandwidth, microwavePsd);
...
main(int argc, char *argv[])
{
  ...
  std::string traceFile = "data-duty";

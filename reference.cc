...
double g_throughputScaleFactor = 1.7;
...
CollectAndSendMetrics()
{
    ...
    // Only trigger emergency reconnect for STAs that cannot hear their AP (rssi_sta=0)
    // Pass 0.0 for packet loss to disable zombie detection (which caused cycling)
    for (uint32_t i = 0; i < g_staChannelHoppingManagers.size(); i++) {
        auto manager = g_staChannelHoppingManagers[i];
        if (manager) {
            // Pass 0.0 for packet loss to disable zombie detection
...
main(int argc, char* argv[])
{
    ...
    // FIX: Configure association manager to allow all channel widths
    // This prevents NS_ABORT during emergency reconnects when STA switches channels
    ...
    // CRITICAL FIX: Uninstall traffic control layer on ALL devices
    // This allows WiFi MAC queue to provide proper backpressure to applications
    ...
    for (uint32_t i = 0; i < numStas; i++)
    {
        ...
        // AIMD: backoff to 50% on failure, increase 100Kbps after 10 successes
        adaptive.SetAttribute("BackoffMultiplier", DoubleValue(0.5));
        adaptive.SetAttribute("AdditiveIncrease", UintegerValue(100000));
        adaptive.SetAttribute("SuccessThreshold", UintegerValue(10));

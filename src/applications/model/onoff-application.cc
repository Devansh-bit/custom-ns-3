...
OnOffApplication::StartApplication() // Called at time specified by Start
{
    ...
    if (!m_socket)
    {
        ...
        // This fixes the deadlock issue when Send() persistently fails due to full queues
        m_socket->SetSendCallback(MakeCallback(&OnOffApplication::DataSend, this));
...
void
OnOffApplication::DataSend(Ptr<Socket> socket, uint32_t availableBufferSize)
{
    NS_LOG_FUNCTION(this << socket << availableBufferSize);

    // Only retry if we have a cached unsent packet and we're in connected state
    // This callback is invoked when buffer space becomes available
    if (m_unsentPacket && m_connected)
    {
        NS_LOG_DEBUG("Buffer space available (" << availableBufferSize
                     << " bytes), retrying cached packet send");
        // Retry sending the cached packet
        SendPacket();
    }
}

...
/**
 * @def NS3_DISABLE_TIME_MARKING
 * @brief Define this macro to disable Time object marking for performance.
 *
 * The Time marking system tracks Time objects created before SetResolution()
 * is called, allowing them to be converted to the new resolution. If your
 * simulation calls Time::SetResolution() early (before creating Time objects),
 * you can define NS3_DISABLE_TIME_MARKING to eliminate the overhead of the
 * MarkingTimes() check on every Time constructor/destructor (~4-5% CPU savings).
 *
 * Enable via: ./ns3 configure --enable-examples --build-profile=optimized
 *             -DNS3_DISABLE_TIME_MARKING
 * Or add to CXXFLAGS: -DNS3_DISABLE_TIME_MARKING
 */
...
class CORE_EXPORT Time
{
  public:
    ...
    inline Time()
        : m_data()
    {
#ifndef NS3_DISABLE_TIME_MARKING
        if (MarkingTimes())
        {
            Mark(this);
        }

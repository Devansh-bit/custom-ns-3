...
TcpSocketBase::SendEmptyPacket(uint8_t flags)
{
    ...
    if (hasSyn)
    {
        ...
        { // Linear backoff of connection time out
            int retryCount = m_synRetries - m_synCount;
            m_rto = m_cnTimeout + (retryCount * m_minRto);
            m_synCount--;
        }
...
TcpSocketBase::NewAck(const SequenceNumber32& ack, bool resetRTO)
{
    ...
    // Reset initial RTO for linear backoff on successful ACK
    m_initialRto = Time::Max();
...
TcpSocketBase::ReTxTimeout()
{
    ...
    // DEATH-PROOF TCP: Never give up on retries - keep trying forever
    // This is needed for long-running simulations with mobile STAs
    // The connection will retry indefinitely until connectivity is restored
    if (m_dataRetrCount > 0)
    {
        --m_dataRetrCount;
    }
    // NOTE: Removed the "drop connection" code - TCP will retry indefinitely
    // Original code would call NotifyErrorClose() and DeallocateEndPoint() when m_dataRetrCount == 0
    ...
    // Linear backoff: store initial RTO on first timeout, then add it on each retry
    if (m_initialRto == Time::Max())
    {
        // First timeout - store the initial RTO value
        m_initialRto = m_rto;
    }
    // Add initial RTO for linear backoff (instead of doubling)
    Time linearRto = m_rto + m_initialRto;
    m_rto = Min(linearRto, Time::FromDouble(5, Time::S));  // Cap at 5 seconds for faster recovery

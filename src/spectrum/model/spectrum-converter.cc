...
{
    NS_LOG_FUNCTION(this);
    double overlap = std::min(from.fh, to.fh) - std::max(from.fl, to.fl);
    overlap = std::max(0.0, overlap);
    double fromWidth = from.fh - from.fl;
    double toWidth = to.fh - to.fl;
    double coeff = std::min(1.0, overlap / toWidth);

    // Add detailed logging to diagnose asymmetry
    if (overlap > 0)
    {
        NS_LOG_INFO("SpectrumConverter Coefficient Calculation:"
                    << " | From: [" << from.fl/1e6 << "-" << from.fh/1e6 << "] MHz (width=" << fromWidth/1e6 << " MHz)"
                    << " | To: [" << to.fl/1e6 << "-" << to.fh/1e6 << "] MHz (width=" << toWidth/1e6 << " MHz)"
                    << " | Overlap: " << overlap/1e6 << " MHz"
                    << " | Coeff: " << coeff
                    << " | Asymmetric: normalizing by TO width only");
    }

...
// Static storage for channel utilization (BSSID -> utilization 0-255)
static std::map<Mac48Address, uint8_t> g_channelUtilization;
// Static storage for WiFi channel utilization (BSSID -> utilization 0-255)
static std::map<Mac48Address, uint8_t> g_wifiChannelUtilization;
// Static storage for non-WiFi channel utilization (BSSID -> utilization 0-255)
static std::map<Mac48Address, uint8_t> g_nonWifiChannelUtilization;

// Static method to set channel utilization for an AP (0-255 scale)
void
ApWifiMac::SetChannelUtilization(Mac48Address bssid, uint8_t utilization)
{
    NS_LOG_INFO("SetChannelUtilization: BSSID=" << bssid << " util=" << (int)utilization);
    g_channelUtilization[bssid] = utilization;
}

// Static method to get channel utilization for an AP (0-255 scale for BSS Load IE)
uint8_t
ApWifiMac::GetChannelUtilizationScaled(Mac48Address bssid)
{
    auto it = g_channelUtilization.find(bssid);
    if (it == g_channelUtilization.end())
    {
        NS_LOG_WARN("GetChannelUtilizationScaled: BSSID=" << bssid << " NOT FOUND, returning 0");
        return 0;  // No utilization data found
    }
    NS_LOG_INFO("GetChannelUtilizationScaled: BSSID=" << bssid << " util=" << (int)it->second);
    return it->second;
}

// Static method to set WiFi channel utilization for an AP (0-255 scale)
void
ApWifiMac::SetWifiChannelUtilization(Mac48Address bssid, uint8_t utilization)
{
    NS_LOG_INFO("SetWifiChannelUtilization: BSSID=" << bssid << " util=" << (int)utilization);
    g_wifiChannelUtilization[bssid] = utilization;
}

// Static method to set non-WiFi channel utilization for an AP (0-255 scale)
void
ApWifiMac::SetNonWifiChannelUtilization(Mac48Address bssid, uint8_t utilization)
{
    g_nonWifiChannelUtilization[bssid] = utilization;
}

// Static method to get WiFi channel utilization for an AP (0-255 scale for BSS Load IE)
uint8_t
ApWifiMac::GetWifiChannelUtilizationScaled(Mac48Address bssid)
{
    auto it = g_wifiChannelUtilization.find(bssid);
    if (it == g_wifiChannelUtilization.end())
    {
        NS_LOG_WARN("GetWifiChannelUtilizationScaled: BSSID=" << bssid << " NOT FOUND, returning 0");
        return 0;  // No utilization data found
    }
    NS_LOG_INFO("GetWifiChannelUtilizationScaled: BSSID=" << bssid << " util=" << (int)it->second);
    return it->second;
}

// Static method to get non-WiFi channel utilization for an AP (0-255 scale for BSS Load IE)
uint8_t
ApWifiMac::GetNonWifiChannelUtilizationScaled(Mac48Address bssid)
{
    auto it = g_nonWifiChannelUtilization.find(bssid);
    if (it == g_nonWifiChannelUtilization.end())
    {
        NS_LOG_WARN("GetNonWifiChannelUtilizationScaled: BSSID=" << bssid << " NOT FOUND, returning 0");
        return 0;  // No utilization data found
    }
    NS_LOG_INFO("GetNonWifiChannelUtilizationScaled: BSSID=" << bssid << " util=" << (int)it->second);
    return it->second;
}
...
ApWifiMac::GetProbeRespProfile(uint8_t linkId) const
{
    ...
    if (GetQosSupported())
    {
        ...
        // Add BSS Load Information Element to probe responses (same as beacons)
        BssLoad bssLoad;
        bssLoad.SetStationCount(GetStaList(linkId).size());
        uint8_t channelUtil = GetChannelUtilizationScaled(GetLink(linkId).feManager->GetAddress());
        bssLoad.SetChannelUtilization(channelUtil);
        // Get WiFi and non-WiFi utilization (packed into AAC field)
        uint8_t wifiUtil = GetWifiChannelUtilizationScaled(GetLink(linkId).feManager->GetAddress());
        uint8_t nonWifiUtil = GetNonWifiChannelUtilizationScaled(GetLink(linkId).feManager->GetAddress());
        bssLoad.SetWifiUtilization(wifiUtil);
        bssLoad.SetNonWifiUtilization(nonWifiUtil);
        probe.Get<BssLoad>() = bssLoad;
    }
    if (GetHtSupported(linkId))
    {
...
ApWifiMac::SetAid(MgtAssocResponseHeader& assoc, const LinkIdStaAddrMap& linkIdStaAddrMap)
{
    ...
    // IEEE 802.11 association cleanup for cross-AP roaming
    for (auto& [id, link] : GetLinks())
    {
        auto& staList = GetLink(id).staList;
        for (auto it = staList.begin(); it != staList.end(); )
        {
            if (it->second == addr || it->second == staAddr)
            {
                auto oldAid = it->first;
                NS_LOG_WARN("AP " << GetAddress() << ": Cleaning stale association for "
                            << it->second << " (AID " << oldAid << ") before reassociation");

                // Clean up all data structures
                m_aidToMldOrLinkAddress.erase(oldAid);
                GetLink(id).stationManager->SetAssociationId(it->second, SU_STA_ID);
                it = staList.erase(it);
            }
            else
            {
    ...
    // FIX #4: Check for and clean up stale m_aidToMldOrLinkAddress entries
    // This prevents crashes when data structures get out of sync
    auto existingMapping = m_aidToMldOrLinkAddress.find(aid);
    if (existingMapping != m_aidToMldOrLinkAddress.end())
    {
        if (existingMapping->second != addr)
        {
            NS_LOG_WARN("AP " << GetAddress() << ": AID " << aid
                        << " already mapped to " << existingMapping->second
                        << ", cleaning up for reassignment to " << addr);
            m_aidToMldOrLinkAddress.erase(existingMapping);
        }
    }
    ...
    {
        NS_LOG_ERROR("AP " << GetAddress() << ": AID " << aid
                     << " still present after cleanup, cannot assign to " << addr);
        NS_ABORT_MSG("AID " << aid << " already present, cannot be assigned to " << addr);
    }
...
ApWifiMac::SendOneBeacon(uint8_t linkId)
{
    ...
    if (GetQosSupported())
    {
        ...
        // Add BSS Load Information Element
        BssLoad bssLoad;
        bssLoad.SetStationCount(GetStaList(linkId).size());
        // Get actual channel utilization from CCA monitor (0-255 scale)
        uint8_t channelUtil = GetChannelUtilizationScaled(link.feManager->GetAddress());
        bssLoad.SetChannelUtilization(channelUtil);
        // Get WiFi and non-WiFi utilization (packed into AAC field)
        uint8_t wifiUtil = GetWifiChannelUtilizationScaled(link.feManager->GetAddress());
        uint8_t nonWifiUtil = GetNonWifiChannelUtilizationScaled(link.feManager->GetAddress());
        bssLoad.SetWifiUtilization(wifiUtil);
        bssLoad.SetNonWifiUtilization(nonWifiUtil);
        beacon.Get<BssLoad>() = bssLoad;

        NS_LOG_INFO("BSS Load IE: BSSID=" << link.feManager->GetAddress()
                    << " StaCount=" << GetStaList(linkId).size()
                    << " ChannelUtil=" << (int)channelUtil << "/255"
                    << " WiFiUtil=" << (int)wifiUtil << "/255"
                    << " NonWiFiUtil=" << (int)nonWifiUtil << "/255");
    }
    if (GetHtSupported(linkId))
    {
        ...
        else if (hdr->GetAddr1() == GetFrameExchangeManager(linkId)->GetAddress())
        {
            ...
            case WIFI_MAC_MGT_DISASSOCIATION: {
                ...
                for (auto it = staList.begin(); it != staList.end(); ++it)
                {
                    if (it->second == from)
                    {
                        auto aid = it->first;  // Get the AID
                        auto address = it->second;

                        // IEEE 802.11 disassociation: clear AID from StationManager
                        GetWifiRemoteStationManager(linkId)->SetAssociationId(from, SU_STA_ID);
                        GetWifiRemoteStationManager(linkId)->RecordDisassociated(from);

                        // Free the AID mapping
                        m_aidToMldOrLinkAddress.erase(aid);
...
bool
ApWifiMac::DeassociateSta(Mac48Address addr, uint8_t linkId)
{
    NS_LOG_FUNCTION(this << addr << linkId);

    auto& staList = GetLink(linkId).staList;

    // Find the STA in the list by MAC address
    for (auto it = staList.begin(); it != staList.end(); ++it)
    {
        if (it->second == addr)
        {
            ...
            NS_LOG_INFO("DeassociateSta: Removing STA " << addr << " (AID=" << aid
                        << ") from link " << +linkId);

            // Clear AID from StationManager
            GetWifiRemoteStationManager(linkId)->SetAssociationId(addr, SU_STA_ID);
            GetWifiRemoteStationManager(linkId)->RecordDisassociated(addr);
            ...
            // Fire the deassociation logger
            m_deAssocLogger(aid, address);

            // Update station counters
            if (GetWifiRemoteStationManager(linkId)->GetDsssSupported(addr) &&
                !GetWifiRemoteStationManager(linkId)->GetErpOfdmSupported(addr))
            {
                GetLink(linkId).numNonErpStations--;
            }
            if (!GetWifiRemoteStationManager(linkId)->GetHtSupported(addr) &&
                !GetWifiRemoteStationManager(linkId)->GetStationHe6GhzCapabilities(addr))
            {
                GetLink(linkId).numNonHtStations--;
            }
            ...
            UpdateShortSlotTimeEnabled(linkId);
            UpdateShortPreambleEnabled(linkId);
            StaSwitchingToActiveModeOrDeassociated(addr, linkId);

            if (m_gcrManager)
            {
                m_gcrManager->NotifyStaDeassociated(addr);
            }
    ...
    NS_LOG_DEBUG("DeassociateSta: STA " << addr << " not found in staList");
    return false;
}
...
void
ApWifiMac::SwitchChannel(uint16_t channelNumber, uint8_t linkId)
{
    NS_LOG_FUNCTION(this << channelNumber << +linkId);

    // IEEE 802.11 Channel Switch Announcement: preserve STA associations
    const std::map<uint16_t, Mac48Address>& staListRef = GetStaList(linkId);

    // Copy unique MAC addresses to a set before clearing staList
    std::set<Mac48Address> uniqueStaAddresses;
    for (const auto& staPair : staListRef)
    {
        uniqueStaAddresses.insert(staPair.second);
    }

    NS_LOG_UNCOND("  AP propagating channel change to " << uniqueStaAddresses.size() << " connected STAs");

    // IEEE 802.11 CSA: preserve association state during coordinated switch
    if (!IsCoordinatedChannelSwitch())
    {
        // Only clear association state for uncoordinated switches
        GetLink(linkId).staList.clear();
        m_aidToMldOrLinkAddress.clear();
        NS_LOG_UNCOND("Cleared staList and AID map - STAs will reassociate on new channel");
    }
    else
    {
        NS_LOG_INFO("Preserving staList and AID mappings during coordinated channel switch on link " << +linkId);
    }

    // Build the channel settings string
    // Determine band from channel number, use width=0 for auto-width detection
    uint8_t primary20 = 0;
    ...
    if (channelNumber >= 1 && channelNumber <= 14)
    {
        ...
        width = 20; // Must be explicit for 2.4 GHz to avoid ambiguity (DSSS 22MHz, OFDM 20MHz, OFDM 40MHz)
    }
    else
    {
    ...
    // FIX: Schedule channel switches to avoid spectrum model mismatch with in-flight packets
    // Use a small delay (10ms) to allow any in-flight transmissions to complete before
    // changing the spectrum model. This prevents the race condition where:
    // 1. Packet is transmitted on old channel (e.g., channel 36 spectrum model)
    // 2. PHY switches to new channel (e.g., channel 40 spectrum model)
    // 3. Packet arrives with mismatched spectrum model -> assertion failure
    Time switchDelay = MilliSeconds(10);

    NS_LOG_UNCOND("  Scheduling coordinated channel switch in " << switchDelay.As(Time::MS)
                  << " to allow in-flight packets to complete");

    // Schedule AP's PHY channel switch
    auto phy = GetLink(linkId).phy;
    if (phy)
    {
        Simulator::Schedule(switchDelay, [phy, channelSettings, this]() {
            phy->SetAttribute("ChannelSettings", StringValue(channelSettings));
            NS_LOG_DEBUG("Node " << GetDevice()->GetNode()->GetId()
                                  << ": Applied ChannelSettings = " << channelSettings);
        });
    }

    // Schedule all STA PHY channel switches to happen at the SAME time as the AP
    // This ensures atomic spectrum model updates across all devices
    for (const auto& staAddress : uniqueStaAddresses)
    {
        // Find the STA node by iterating through all nodes
        for (uint32_t i = 0; i < NodeList::GetNNodes(); ++i)
        {
            ...
            for (uint32_t j = 0; j < node->GetNDevices(); ++j)
            {
                ...
                if (wifiDev && wifiDev->GetAddress() == staAddress)
                {
                    // Found the STA - set coordinated flag on its MAC and schedule channel change
                    Ptr<WifiMac> staMac = wifiDev->GetMac();
                    Ptr<WifiPhy> staPhy = wifiDev->GetPhy();
                    
                    if (staMac && staPhy)
                    {
                        uint32_t nodeId = node->GetId();
                        
                        // IEEE 802.11 CSA: STA follows AP to new channel without reassociation
                        staMac->SetCoordinatedChannelSwitch(IsCoordinatedChannelSwitch());
                        
                        Simulator::Schedule(switchDelay, [staPhy, channelSettings, nodeId]() {
                            staPhy->SetAttribute("ChannelSettings", StringValue(channelSettings));
                            NS_LOG_UNCOND("    ✓ Applied to STA node " << nodeId);
                        });
                        
                        // Reset STA's coordinated flag after 500ms (same as AP)
                        Simulator::Schedule(switchDelay + MilliSeconds(500), [staMac]() {
                            staMac->SetCoordinatedChannelSwitch(false);
                        });
                    }
                    break;
                }

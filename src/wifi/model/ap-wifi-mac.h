...
class ApWifiMac : public WifiMac
{
  public:
    ...
    /**
     * @brief Set channel utilization for an AP (for BSS Load IE)
     * @param bssid The BSSID of the AP
     * @param utilization Channel utilization (0-255 scale)
     */
    static void SetChannelUtilization(Mac48Address bssid, uint8_t utilization);

    /**
     * @brief Get channel utilization for an AP (0-255 scale for BSS Load IE)
     * @param bssid The BSSID of the AP
     * @return Channel utilization (0-255 scale)
     */
    static uint8_t GetChannelUtilizationScaled(Mac48Address bssid);

    /**
     * @brief Set WiFi channel utilization for an AP (for BSS Load IE)
     * @param bssid The BSSID of the AP
     * @param utilization WiFi channel utilization (0-255 scale)
     */
    static void SetWifiChannelUtilization(Mac48Address bssid, uint8_t utilization);

    /**
     * @brief Set non-WiFi channel utilization for an AP (for BSS Load IE)
     * @param bssid The BSSID of the AP
     * @param utilization Non-WiFi channel utilization (0-255 scale)
     */
    static void SetNonWifiChannelUtilization(Mac48Address bssid, uint8_t utilization);

    /**
     * @brief Get WiFi channel utilization for an AP (0-255 scale for BSS Load IE)
     * @param bssid The BSSID of the AP
     * @return WiFi channel utilization (0-255 scale)
     */
    static uint8_t GetWifiChannelUtilizationScaled(Mac48Address bssid);

    /**
     * @brief Get non-WiFi channel utilization for an AP (0-255 scale for BSS Load IE)
     * @param bssid The BSSID of the AP
     * @return Non-WiFi channel utilization (0-255 scale)
     */
    static uint8_t GetNonWifiChannelUtilizationScaled(Mac48Address bssid);
    ...
    /**
     * @brief Force deassociate a STA from this AP
     *
     * This method removes the STA from the AP's internal station list without
     * sending a disassociation frame. Useful for handling roaming scenarios
     * where the STA has already associated with another AP.
     *
     * @param addr the MAC address of the station to deassociate
     * @param linkId the ID of the link (default 0)
     * @return true if the STA was found and removed, false otherwise
     */
    bool DeassociateSta(Mac48Address addr, uint8_t linkId = 0);
     ...
     *
     * This override atomically changes the AP and all connected STAs' channels.
     *
     * @note Known limitation: In ns-3, changing PHY channels while STAs are associated
     * may trigger re-associations that add duplicate entries to the internal STA list.
     * This is a simulator artifact and does not affect actual operation, but may cause
     * the STA list size to grow with repeated channel switches.
     */

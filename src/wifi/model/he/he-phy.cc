...
HePhy::SwitchMaybeToCcaBusy(const Ptr<const WifiPpdu> ppdu)
{
    ...
    // If ppdu is not null, a WiFi preamble was detected (WiFi signal)
    // If ppdu is null, only energy was detected (non-WiFi interference)
    bool wifiCaused = (ppdu != nullptr);
    if (ccaIndication.has_value())
    {
        NS_LOG_DEBUG("CCA busy for " << ccaIndication.value().second << " during "
                                     << ccaIndication.value().first.As(Time::S)
                                     << " wifiCaused=" << wifiCaused);
        NotifyCcaBusy(ccaIndication.value().first, ccaIndication.value().second, per20MHzDurations, wifiCaused);
        return;
    }
    if (ppdu)
    {
    ...
    if (per20MHzDurations != m_lastPer20MHzDurations)
    {
        ...
        NotifyCcaBusy(Seconds(0), WIFI_CHANLIST_PRIMARY, per20MHzDurations, false);
    }
}
...
HePhy::NotifyCcaBusy(const Ptr<const WifiPpdu> ppdu, Time duration, WifiChannelListType channelType)
{
    ...
    // If ppdu is null, only energy was detected (non-WiFi interference)
    bool wifiCaused = (ppdu != nullptr);
    NS_LOG_FUNCTION(this << duration << channelType << wifiCaused);
    NS_LOG_DEBUG("CCA busy for " << channelType << " during " << duration.As(Time::S)
                                 << " wifiCaused=" << wifiCaused);
    const auto per20MHzDurations = GetPer20MHzDurations(ppdu);
    NotifyCcaBusy(duration, channelType, per20MHzDurations, wifiCaused);
}
...
{
    NS_LOG_FUNCTION(this << duration << channelType << wifiCaused);
    m_state->SwitchMaybeToCcaBusy(duration, channelType, per20MHzDurations, wifiCaused);
...
HePhy::GetDataRate(uint8_t mcsValue, MHz_u channelWidth, Time guardInterval, uint8_t nss)
{
    ...
    NS_ASSERT((gi == 800) || (gi == 1600) || (gi == 3200));
    NS_ASSERT(nss <= 8);
    uint64_t rate = HtPhy::CalculateDataRate(GetSymbolDuration(guardInterval),
                                    ...
                                    nss);
    // Reduce data rate to inflate channel utilization (scale factor = 10)
    return rate / 10;
}

                                          ...
                                          Mac48Address originator)
{
    ...
    // Check if HT is supported for the originator before proceeding
    // This prevents assertion failures when ADDBA is received before capabilities are exchanged
    if (!m_mac->GetHtSupported(originator) && !m_mac->GetHeSupported(originator) &&
        !m_mac->GetEhtSupported(originator))
    {
        NS_LOG_WARN("Ignoring ADDBA Request from " << originator
                    << " - HT/HE/EHT capabilities not yet known");
        return;
    }

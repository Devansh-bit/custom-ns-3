...
#include "../helper/spectrum-power-cache.h" // Phase 3D: for cache invalidation
...
SpectrumWifiPhy::SpectrumWifiPhy()
    : m_spectrumPhyInterfaces{},
      ...
      m_channelConfigVersion{0} // Phase 3D: version tracking for cache invalidation
{
...
SpectrumWifiPhy::FinalizeChannelSwitch()
{
    ...
    if ((m_currentSpectrumPhyInterface->GetCenterFrequencies() == frequenciesAfter) &&
        (m_currentSpectrumPhyInterface->GetChannelWidth() == GetOperatingChannel().GetTotalWidth()))
    {
        // Center frequencies and channel width have not changed for that interface, hence we do
        // not need to reset the spectrum model nor update any band stored in the interference helper
...
{
    // Phase 3D: Increment version for cache invalidation
    m_channelConfigVersion++;

    // Phase 3D: Clear GetBandPowerW cache on channel switch
    // This is safe because channel switches are infrequent
                         ...
                         Ptr<const WifiSpectrumPhyInterface> interface)
{
    ...
    if (interface)
    {
        // FIX: Replace hard assertion with graceful error handling
        // This can occur during channel switching when in-flight packets arrive
        // after the receiver has already switched to a new channel/spectrum model
        if (receivedSignalPsd->GetValuesN() != interface->GetRxSpectrumModel()->GetNumBands())
        {
            NS_LOG_WARN("Spectrum model mismatch detected - dropping packet. "
                       << "Received signal has " << receivedSignalPsd->GetValuesN() << " bands, "
                       << "but receiver expects " << interface->GetRxSpectrumModel()->GetNumBands() << " bands. "
                       << "This can occur during channel switching when packets transmitted on the old channel "
                       << "arrive after the PHY has switched to a new channel.");
            return; // Drop the packet instead of crashing
        }
    }
    ...
    // Phase 3A: Batch band power calculation for performance
    // Collect all bands first, then compute all powers in one call
    ...
    // Collect all band indices for batch processing
    std::vector<std::vector<WifiSpectrumBandIndices>> allBandIndices;
    std::vector<const WifiSpectrumBandInfo*> bandInfoPtrs; // Keep track of which band is which
    allBandIndices.reserve(bands.size() + 50); // Reserve space (bands + potential RU bands)
    bandInfoPtrs.reserve(bands.size() + 50);

    for (const auto& band : bands)
    {
        allBandIndices.push_back(band.indices);
        bandInfoPtrs.push_back(&band);
    }

    // Add RU bands if 802.11ax+
    std::vector<std::pair<WifiSpectrumBandInfo, WifiRu::RuSpec>> ruBandsVec;
    if (GetStandard() >= WIFI_STANDARD_80211ax)
    {
        const auto& ruBands =
        ...
        {
            allBandIndices.push_back(band.indices);
            ruBandsVec.push_back({band, ru});
        }
    }

    // Phase 3A: Single batch call instead of N individual calls
    auto bandPowers = WifiSpectrumValueHelper::GetBandPowerWBatch(receivedSignalPsd, allBandIndices);
    ...
    {
        const auto& band = *bandInfoPtrs[i];
        auto rxPowerPerBand = bandPowers[i];
    ...
    // Process RU bands (already computed in batch above)
    if (GetStandard() >= WIFI_STANDARD_80211ax)
    {
        for (size_t i = 0; i < ruBandsVec.size(); ++i)
        {
            const auto& [band, ru] = ruBandsVec[i];
            auto rxPowerPerBand = bandPowers[bands.size() + i];
            rxPowerPerBand *= rxGainRatio;
            rxPowers.insert({band, rxPowerPerBand});
        }
    }

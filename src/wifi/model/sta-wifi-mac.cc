...
StaWifiMac::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::StaWifiMac")
            ...
            .AddAttribute("BackgroundScanInterval",
                          "Time interval between background scans while associated",
                          TimeValue(Seconds(5.0)),
                          MakeTimeAccessor(&StaWifiMac::m_backgroundScanInterval),
                          MakeTimeChecker())
            .AddAttribute("ScanResultMaxAge",
                          "Maximum age of cached scan results before they are removed",
                          TimeValue(Seconds(30.0)),
                          MakeTimeAccessor(&StaWifiMac::m_scanResultMaxAge),
                          MakeTimeChecker())
            .AddTraceSource("RoamingInitiated",
                            "Roaming to a new AP has been initiated. Provides current time, old BSSID, "
                            "new BSSID, current RSSI (dBm), and current SNR (linear)",
                            MakeTraceSourceAccessor(&StaWifiMac::m_roamingInitiatedLogger),
                            "ns3::StaWifiMac::RoamingInitiatedCallback")
            .AddTraceSource("RoamingCompleted",
                            "Roaming to a new AP has completed. Provides current time, new BSSID, "
                            "roaming latency, and success flag",
                            MakeTraceSourceAccessor(&StaWifiMac::m_roamingCompletedLogger),
                            "ns3::StaWifiMac::RoamingCompletedCallback")
            .AddTraceSource("BackgroundScanComplete",
                            "Background scan has completed. Provides number of APs found and map "
                            "of BSSID to RSSI (dBm)",
                            MakeTraceSourceAccessor(&StaWifiMac::m_backgroundScanCompleteLogger),
                            "ns3::StaWifiMac::BackgroundScanCompleteCallback")
            .AddTraceSource("LinkQualityUpdate",
                            "Link quality metrics updated. Provides current time, BSSID, RSSI (dBm), "
                            "SNR (linear), retry count, and throughput (Mbps)",
                            MakeTraceSourceAccessor(&StaWifiMac::m_linkQualityUpdateLogger),
                            "ns3::StaWifiMac::LinkQualityUpdateCallback");
    return tid;
}
...
StaWifiMac::TryToEnsureAssociated()
{
    ...
    case UNASSOCIATED:
        ...
        break;
    case ROAMING:
        /* we are in the process of roaming to a different AP.
         * Wait for the roaming process to complete.
         */
        break;
...
StaWifiMac::ScanningTimeout(const std::optional<ApInfo>& bestAp)
{
    ...
    // Check if we're still in scanning state - if not, abort
    // (e.g., if AssociateToAp() was called and already initiated association)
    if (m_state != SCANNING)
    {
        NS_LOG_DEBUG("No longer in SCANNING state (current state: " << m_state
                     << "), ignoring ScanningTimeout");
        return;
    }
...
StaWifiMac::ReceiveBeacon(Ptr<const WifiMpdu> mpdu, uint8_t linkId)
{
    ...
    if (m_state == ASSOCIATED)
    {
        ...
        // Update link quality for current AP
        // Calculate RSSI from SNR
        double noiseFloorDbm = -90.0;
        double rssi = (apInfo.m_snr > 0) ? (10.0 * std::log10(apInfo.m_snr) + noiseFloorDbm) : noiseFloorDbm;
        UpdateLinkQuality(hdr.GetAddr3(), rssi, apInfo.m_snr);
    }
    else if (m_enableBackgroundScanning && IsAssociated())
    {
        // Process beacons from other APs for background scanning
        ProcessBackgroundScanResult(apInfo);
    }
    else
    {
...
StaWifiMac::ReceiveAssocResp(Ptr<const WifiMpdu> mpdu, uint8_t linkId)
{
    ...
    if (m_state != WAIT_ASSOC_RESP && m_state != ROAMING)
    {
        return;
    }

    // Remember if we were roaming
    bool wasRoaming = (m_state == ROAMING);
    ...
    if (assocResp.GetStatusCode().IsSuccess())
    {
        ...
        // Complete roaming if we were in roaming state
        if (wasRoaming)
        {
            CompleteRoaming();
        }
...
StaWifiMac::NotifyChannelSwitching(uint8_t linkId)
{
    ...
    // IEEE 802.11 CSA: maintain association during coordinated channel switch
    if (IsInitialized() && IsAssociated() && !IsCoordinatedChannelSwitch())
    {
        // Only disassociate if NOT coordinated
        Disassociated();
    }
    else if (IsCoordinatedChannelSwitch())
    {
        NS_LOG_INFO("Preserving STA association during coordinated channel switch on link " << +linkId);
    }
...
// ========================================================================
// Manual Roaming Control Implementation
// ========================================================================

void
StaWifiMac::InitiateRoaming(const Mac48Address& targetBssid, uint8_t channel, WifiPhyBand band)
{
    NS_LOG_FUNCTION(this << targetBssid << +channel << band);

    if (!IsAssociated())
    {
        NS_LOG_INFO("STA not associated during InitiateRoaming - falling back to direct association to " << targetBssid);
        // Use AssociateToAp for direct association (sends regular assoc request, not reassoc)
        AssociateToAp(targetBssid, channel, band);
        return;
    }

    // Get current AP BSSID before we start roaming
    auto currentBssidOpt = GetCurrentBssid();
    if (!currentBssidOpt.has_value())
    {
        NS_LOG_WARN("Cannot initiate roaming: no current BSSID");
        return;
    }
    Mac48Address currentBssid = *currentBssidOpt;

    // Save roaming state
    m_previousBssid = currentBssid;
    m_roamingTargetBssid = targetBssid;
    m_roamingStartTime = Simulator::Now();

    // Fire roaming initiated trace
    m_roamingInitiatedLogger(Simulator::Now(),
                              m_previousBssid,
                              targetBssid,
                              m_currentRssi,
                              m_currentSnr);

    NS_LOG_INFO("Initiating roaming from " << m_previousBssid << " to " << targetBssid);

    // Check if we're roaming to a DIFFERENT AP (cross-AP roaming)
    // If so, send deassociation frame to old AP BEFORE switching channels
    // This ensures the old AP removes us from its staList (critical for cross-channel roaming)
    if (currentBssid != targetBssid && !currentBssid.IsGroup())
    {
        NS_LOG_INFO("InitiateRoaming: Sending deassociation to old AP " << currentBssid
                    << " before switching to " << targetBssid);

        // Reason code 8 = "Disassociated because sending STA is leaving (or has left) BSS"
        SendDeassociation(currentBssid, 8);

        // Schedule actual roaming after a short delay to allow deassoc frame transmission
        // 50ms should be enough for the frame to be transmitted on the current channel
        Simulator::Schedule(MilliSeconds(50), &StaWifiMac::DoInitiateRoaming, this,
                            targetBssid, channel, band);
        return;
    }

    // Same AP or no valid current BSSID - proceed directly to roaming
    DoInitiateRoaming(targetBssid, channel, band);
}

void
StaWifiMac::AssociateToAp(const Mac48Address& targetBssid, uint8_t channel, WifiPhyBand band)
{
    NS_LOG_FUNCTION(this << targetBssid << +channel << band);

    // Force association - clear any blocking state first
    if (IsAssociated())
    {
        NS_LOG_INFO("AssociateToAp: Force disassociating from current AP to associate with " << targetBssid);
        // Disable auto-reconnect to prevent scanning race
        m_autoReconnect = false;
        // Clear association state without triggering callbacks that start scanning
        for (auto& [id, link] : GetLinks())
        {
    ...
    {
        NS_LOG_INFO("AssociateToAp: Cancelling pending association to associate with " << targetBssid);
        m_assocRequestEvent.Cancel();
    }

    if (m_state == SCANNING)
    {
        NS_LOG_INFO("AssociateToAp: Cancelling scan to associate with " << targetBssid);
        // Scanning will be overridden when we set state to WAIT_ASSOC_RESP
    }

    NS_LOG_INFO("Associating to AP " << targetBssid << " on channel " << +channel);

    // Switch PHY to target channel/band
    auto& link = GetLink(SINGLE_LINK_OP_ID);
    Ptr<WifiPhy> phy = link.phy;
    if (phy)
    {
        uint8_t currentChannel = phy->GetChannelNumber();
        WifiPhyBand currentBand = phy->GetPhyBand();

        if (currentChannel != channel || currentBand != band)
        {
            NS_LOG_INFO("Switching PHY from channel " << +currentChannel << " to " << +channel);
            ...
            // Apply channel settings via attribute
            phy->SetAttribute("ChannelSettings", StringValue(oss.str()));
            NS_LOG_INFO("Channel switched to " << +phy->GetChannelNumber());
        }
    }

    // Configure link for association - follow same pattern as InitiateRoaming()
    for (auto& [id, link] : GetLinks())
    {
        auto& staLink = GetStaLink(link);
        staLink.sendAssocReq = false;
        staLink.bssid = targetBssid;
    }

    // Set sendAssocReq flag for the first link
    auto firstLink = GetLinks().begin();
    if (firstLink != GetLinks().end())
    {
        GetStaLink(firstLink->second).sendAssocReq = true;
    }

    // Set state to WAIT_ASSOC_RESP and send association request
    SetState(WAIT_ASSOC_RESP);
    SendAssociationRequest(false); // false = initial association (not reassociation)
}

void
StaWifiMac::ForcedDisassociate()
{
    NS_LOG_FUNCTION(this);

    if (!IsAssociated())
    {
        NS_LOG_WARN("Cannot disassociate: STA is not associated");
        return;
    }
    ...
    NS_LOG_INFO("Forced disassociation completed, auto-reconnect disabled");
}

void
StaWifiMac::EnableAutoReconnect(bool enable)
{
    NS_LOG_FUNCTION(this << enable);
    m_autoReconnect = enable;

    if (enable && m_state == UNASSOCIATED)
    {
        // Restart association process
        TryToEnsureAssociated();
    }
}

double
StaWifiMac::GetCurrentRssi() const
{
    if (!IsAssociated())
    {
        return 0.0;
...
double
StaWifiMac::GetCurrentSnr() const
{
    if (!IsAssociated())
    {
        return 0.0;
...
StaWifiMac::GetCurrentBssid() const
{
    if (!IsAssociated())
    {
        return std::nullopt;
    ...
    // Return BSSID from the first associated link
    for (const auto& [id, link] : GetLinks())
    {
        const auto& staLink = GetStaLink(link);
        if (staLink.bssid.has_value())
        {
            return staLink.bssid;
        }
    }
...
void
StaWifiMac::EnableBackgroundScanning(bool enable)
{
    NS_LOG_FUNCTION(this << enable);
    m_enableBackgroundScanning = enable;

    if (enable && IsAssociated())
    {
        ScheduleBackgroundScan();
    }
    else if (!enable && m_backgroundScanEvent.IsPending())
    {
        Simulator::Cancel(m_backgroundScanEvent);
    }
}

void
StaWifiMac::ScheduleBackgroundScan()
{
    NS_LOG_FUNCTION(this);

    // Cancel any existing scheduled scan
    if (m_backgroundScanEvent.IsPending())
    {
        Simulator::Cancel(m_backgroundScanEvent);
    }

    if (m_enableBackgroundScanning && IsAssociated())
    {
        m_backgroundScanEvent = Simulator::Schedule(m_backgroundScanInterval,
                                                     &StaWifiMac::PerformBackgroundScan,
                                                     this);
        NS_LOG_DEBUG("Scheduled background scan in " << m_backgroundScanInterval.GetSeconds()
                                                      << " seconds");
    }
}
...
StaWifiMac::PerformBackgroundScan()
{
    ...
    if (!IsAssociated())
    {
        NS_LOG_DEBUG("Skipping background scan: not associated");
        return;
    }

    NS_LOG_INFO("Performing background scan");

    // For now, we implement a passive background scan
    // Real implementation would:
    // 1. Save current channel
    // 2. Iterate through channels (briefly switch and listen for beacons)
    // 3. Return to original channel
    //
    // Simplified: We rely on beacons received from other APs on the same channel
    // and scan results populated by the ReceiveBeacon method

    // Remove stale entries from scan cache
    Time now = Simulator::Now();
    for (auto it = m_scanResults.begin(); it != m_scanResults.end();)
    {
        if (now - it->second.timestamp > m_scanResultMaxAge)
        {
            NS_LOG_DEBUG("Removing stale scan result for " << it->first);
            it = m_scanResults.erase(it);
        }
        else
        {
            ++it;
    ...
    // Prepare RSSI map for trace
    std::map<Mac48Address, double> rssiMap;
    for (const auto& [bssid, result] : m_scanResults)
    {
        rssiMap[bssid] = result.rssi;
    }

    // Fire background scan complete trace
    m_backgroundScanCompleteLogger(m_scanResults.size(), rssiMap);

    NS_LOG_INFO("Background scan complete: " << m_scanResults.size() << " APs in cache");
...
void
StaWifiMac::TriggerBackgroundScan()
{
    NS_LOG_FUNCTION(this);
    ...
    {
        NS_LOG_WARN("Cannot trigger background scan: not associated");
    }
}

void
StaWifiMac::ProcessBackgroundScanResult(const ApInfo& apInfo)
{
    NS_LOG_FUNCTION(this << apInfo);

    // Create scan result entry
    ScanResult result;
    result.bssid = apInfo.m_bssid;
    result.snr = apInfo.m_snr;
    result.channel = apInfo.m_channel.number;
    result.band = apInfo.m_channel.band;
    result.timestamp = Simulator::Now();

    // Extract RSSI from SNR
    // RSSI (dBm) = 10 * log10(SNR) - NoiseFloor
    // Assuming noise floor of -90 dBm for typical WiFi
    double noiseFloorDbm = -90.0;
    if (apInfo.m_snr > 0)
    {
        result.rssi = 10.0 * std::log10(apInfo.m_snr) + noiseFloorDbm;
    }
    else
    {
        result.rssi = noiseFloorDbm; // Very weak signal
    }
    ...
    std::visit(
        [&result](auto&& frame) {
            ...
            if constexpr (std::is_same_v<T, MgtBeaconHeader> ||
                          std::is_same_v<T, MgtProbeResponseHeader>)
            {
                result.ssid = frame.template Get<Ssid>().value_or(Ssid());
                result.beaconInterval = frame.GetBeaconIntervalUs() / 1024; // Convert to TUs
            }
        },
        apInfo.m_frame);

    // Store in scan results cache
    m_scanResults[apInfo.m_bssid] = result;

    NS_LOG_DEBUG("Cached scan result: BSSID=" << apInfo.m_bssid << ", RSSI=" << result.rssi
                                               << " dBm, Channel=" << +result.channel);
}

std::map<Mac48Address, StaWifiMac::ScanResult>
StaWifiMac::GetScanResults() const
{
    return m_scanResults;
}

void
StaWifiMac::ClearScanResults()
{
    NS_LOG_FUNCTION(this);
    m_scanResults.clear();
}
...
// ========================================================================
// Roaming Helper Methods
// ========================================================================

void
StaWifiMac::CompleteRoaming()
{
    NS_LOG_FUNCTION(this);

    if (m_state != ROAMING)
    {
        NS_LOG_WARN("CompleteRoaming called but state is not ROAMING");
        return;
    }

    // Calculate roaming latency
    Time latency = Simulator::Now() - m_roamingStartTime;

    // Fire roaming completed trace
    m_roamingCompletedLogger(Simulator::Now(), m_roamingTargetBssid, latency, true);

    NS_LOG_INFO("Roaming completed successfully to " << m_roamingTargetBssid << ", latency: "
    ...
    // Clear roaming state
    m_roamingTargetBssid = Mac48Address();
}

void
StaWifiMac::UpdateLinkQuality(const Mac48Address& bssid, double rssi, double snr)
{
    NS_LOG_FUNCTION(this << bssid << rssi << snr);

    m_currentRssi = rssi;
    m_currentSnr = snr;

    // Fire link quality update trace (with placeholder values for retries and throughput)
    // These would be obtained from the frame exchange manager in a complete implementation
    m_linkQualityUpdateLogger(Simulator::Now(), bssid, rssi, snr, 0, 0.0);
}

void
StaWifiMac::SendDeassociation(const Mac48Address& apAddress, uint16_t reasonCode)
{
    NS_LOG_FUNCTION(this << apAddress << reasonCode);

    // Create Disassociation frame header
    WifiMacHeader hdr;
    hdr.SetType(WIFI_MAC_MGT_DISASSOCIATION);
    hdr.SetAddr1(apAddress);     // DA: Destination (AP)
    hdr.SetAddr2(GetAddress());  // SA: Source (this STA)
    hdr.SetAddr3(apAddress);     // BSSID
    hdr.SetDsNotFrom();
    hdr.SetDsNotTo();

    // Create disassociation frame body with reason code (2 bytes, little-endian per IEEE 802.11)
    Ptr<Packet> packet = Create<Packet>();
    uint8_t reasonBytes[2];
    reasonBytes[0] = reasonCode & 0xFF;
    reasonBytes[1] = (reasonCode >> 8) & 0xFF;
    packet->AddAtEnd(Create<Packet>(reasonBytes, 2));

    NS_LOG_INFO("Sending Disassociation to " << apAddress << " with reason code " << reasonCode);

    // Queue to AC_VO (Voice) for priority transmission - same as BSS TM frames
    auto qosTxop = GetQosTxop(AC_VO);
    if (qosTxop)
    {
    ...
    {
        NS_LOG_WARN("SendDeassociation: GetQosTxop(AC_VO) returned null, cannot send frame");
    }
}

void
StaWifiMac::DoInitiateRoaming(const Mac48Address& targetBssid, uint8_t channel, WifiPhyBand band)
{
    NS_LOG_FUNCTION(this << targetBssid << +channel << band);

    // Note: Deassociation was already sent by InitiateRoaming() before calling this method

    // Save current state (if not already saved)
    auto currentBssid = GetCurrentBssid();
    if (currentBssid.has_value() && m_previousBssid.IsGroup())
    {
        m_previousBssid = *currentBssid;
    }
    m_roamingTargetBssid = targetBssid;
    if (m_roamingStartTime.IsZero())
    {
        m_roamingStartTime = Simulator::Now();
    }

    // Fire roaming initiated trace (if not already fired)
    // Note: May have already been fired in InitiateRoaming()

    NS_LOG_INFO("DoInitiateRoaming: Switching from " << m_previousBssid << " to " << targetBssid);

    // Set state to ROAMING
    SetState(ROAMING);

    // Switch PHY to target channel/band if different
    auto& link = GetLink(SINGLE_LINK_OP_ID);
    Ptr<WifiPhy> phy = link.phy;
    if (phy)
    {
        ...
        NS_LOG_DEBUG("DoInitiateRoaming: Current channel: " << +currentChannel
                     << ", Target channel: " << +channel);

        if (currentChannel != channel || currentBand != band)
        {
            NS_LOG_INFO("DoInitiateRoaming: Switching PHY from channel " << +currentChannel << " to " << +channel);
            ...
            // Check PHY state before switching channels
            ...
            if (phyIsBusy)
            {
                ...
                NS_LOG_INFO("DoInitiateRoaming: PHY is busy (" << stateStr << "), retrying in 5ms");
                Simulator::Schedule(MilliSeconds(5), &StaWifiMac::DoInitiateRoaming, this, targetBssid, channel, band);
                return;
            }

            // PHY is idle - safe to switch channel now
            phy->SetAttribute("ChannelSettings", StringValue(oss.str()));
        }
    }

    // Send reassociation request to target AP
    for (auto& [id, link] : GetLinks())
    {
        auto& staLink = GetStaLink(link);
        staLink.sendAssocReq = false;
    ...
    // Change state to WAIT_ASSOC_RESP before sending reassociation request
    SetState(WAIT_ASSOC_RESP);

    SendAssociationRequest(true); // true = reassociation
}

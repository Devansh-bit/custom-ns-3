...
void
WifiMac::SetCoordinatedChannelSwitch(bool coordinated)
{
    NS_LOG_FUNCTION(this << coordinated);
    m_coordinatedChannelSwitch = coordinated;
}

bool
WifiMac::IsCoordinatedChannelSwitch() const
{
    return m_coordinatedChannelSwitch;
}
...
WifiMac::NotifyChannelSwitching(uint8_t linkId)
{
    ...
    // IEEE 802.11 CSA: skip station manager reset during coordinated switch
    if (!IsCoordinatedChannelSwitch())
    {
        // Reset remote station manager only for uncoordinated switches
        GetLink(linkId).stationManager->Reset();
    }
    else
    {
        NS_LOG_DEBUG("Skipping RemoteStationManager reset during coordinated channel switch");
    }
}
...
void
WifiMac::SwitchChannel(uint16_t channelNumber, uint8_t linkId)
{
    ...
    // Channels 1-14: 2.4 GHz, Channels 36+: 5 GHz
    ...
    // Format the channel settings string

...
/**
 * @ingroup wifi
 * Compare two bands for equality.
 *
 * @param lhs the band on the left of operator==
 * @param rhs the band on the right of operator==
 * @return true if both bands have the same frequencies
 */
inline bool
operator==(const WifiSpectrumBandInfo& lhs, const WifiSpectrumBandInfo& rhs)
{
...
// Hash function for WifiSpectrumBandInfo to enable use with std::unordered_map
namespace std
{
template <>
struct hash<ns3::WifiSpectrumBandInfo>
{
    std::size_t operator()(const ns3::WifiSpectrumBandInfo& band) const
    {
        std::size_t h = 0;
        // Hash based on frequency ranges (which uniquely identify bands)
        for (const auto& freq : band.frequencies)
        {
            // Combine hashes using a standard method
            h ^= std::hash<double>{}(freq.first) + 0x9e3779b9 + (h << 6) + (h >> 2);
            h ^= std::hash<double>{}(freq.second) + 0x9e3779b9 + (h << 6) + (h >> 2);
        }
        return h;
    }
};

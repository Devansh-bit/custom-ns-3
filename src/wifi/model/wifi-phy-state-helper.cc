...
WifiPhyStateHelper::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::WifiPhyStateHelper")
            ...
            .AddTraceSource("CcaBusyType",
                            "CCA busy state with WiFi/non-WiFi classification. "
                            "Fired when CCA busy is detected, indicating whether it was "
                            "caused by a WiFi signal (preamble detected) or non-WiFi interference.",
                            MakeTraceSourceAccessor(&WifiPhyStateHelper::m_ccaBusyTypeTrace),
                            "ns3::WifiPhyStateHelper::CcaBusyTypeTracedCallback");
    return tid;
}
                                         ...
                                         bool wifiCaused)
{
    ...
    // Fire the CCA busy type trace with WiFi/non-WiFi classification
    if (duration.IsStrictlyPositive())
    {
        m_ccaBusyTypeTrace(now, duration, wifiCaused);
    }
}

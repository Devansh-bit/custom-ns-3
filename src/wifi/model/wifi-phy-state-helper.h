...
class WIFI_EXPORT WifiPhyStateHelper : public Object
{
  public:
     ...
     * @param wifiCaused true if the CCA busy was caused by a WiFi signal (preamble detected),
     *        false if caused by non-WiFi interference (energy without valid preamble).
     *        Default is false for backward compatibility.
     */
    void SwitchMaybeToCcaBusy(Time duration,
                              WifiChannelListType channelType,
                              const std::vector<Time>& per20MhzDurations,
                              bool wifiCaused = false);
    /**
    ...
    /**
     * TracedCallback signature for CCA busy state with WiFi/non-WiFi classification.
     *
     * @param [in] start Time when the CCA busy state started.
     * @param [in] duration Duration of the CCA busy state.
     * @param [in] wifiCaused true if caused by WiFi signal (preamble detected),
     *             false if caused by non-WiFi interference.
     */
    typedef void (*CcaBusyTypeTracedCallback)(Time start, Time duration, bool wifiCaused);
  ...
  private:
    ...
    TracedCallback<Time, Time, bool>
        m_ccaBusyTypeTrace;            ///< CCA busy type trace (start, duration, wifiCaused)

...
WifiPhy::Send(const WifiConstPsduMap& psdus, const WifiTxVector& txVector)
{
    ...
    // FIX: Defensive check - drop transmission if PHY is busy instead of crashing
    // This can happen with dual-PHY setups or race conditions in MAC scheduling
    if (m_state->IsStateTx() || m_state->IsStateSwitching())
    {
        NS_LOG_WARN("WifiPhy::Send - PHY busy (TX=" << m_state->IsStateTx()
                    << ", Switching=" << m_state->IsStateSwitching()
                    << ") at " << Simulator::Now().As(Time::S)
                    << " on Node " << (m_device ? m_device->GetNode()->GetId() : -1)
                    << " Device " << (m_device ? m_device->GetIfIndex() : -1)
                    << ". Dropping transmission to avoid collision.");

        // Notify TX drop for each PSDU
        for (const auto& psdu : psdus)
        {
            NotifyTxDrop(psdu.second);
        }
        return;  // Drop gracefully instead of crashing
    }

    if (!m_endTxEvent.IsExpired())
    {
        NS_LOG_WARN("WifiPhy::Send - Previous transmission not complete. Dropping packet.");
        for (const auto& psdu : psdus)
        {
            NotifyTxDrop(psdu.second);
        }
...
WifiPhy::GetSubcarrierSpacing() const
{
    ...
    case WIFI_STANDARD_80211n:
    case WIFI_STANDARD_80211ac:
        subcarrierSpacing = SUBCARRIER_FREQUENCY_SPACING_COARSE;
        break;
    case WIFI_STANDARD_80211p:

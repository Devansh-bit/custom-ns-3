...
#include "../helper/spectrum-power-cache.h"
                                                          ...
                                                          dBr_u lowestPoint)
{
    ...
    // 13 subcarriers (scaled from 52 due to 1.25 MHz carrier spacing: 52/4 = 13)
    // skip guard band and 2 subbands, then place power in 7 subbands,
    // then place power in 6 more subbands (no DC gap at coarse granularity),
    // then skip the final 2 subbands and the guard band.
    const auto txPowerPerBand = txPower / 13;
    NS_LOG_DEBUG("Power per band " << txPowerPerBand << "W");
    uint32_t start1 = (nGuardBands / 2) + 2;
    uint32_t stop1 = start1 + 7 - 1;
    uint32_t start2 = stop1 + 1;
    uint32_t stop2 = start2 + 6 - 1;
    ...
    const std::vector<bool>& puncturedSubchannels)
{
    ...
    NS_LOG_FUNCTION(printFrequencies(centerFrequencies)
                    ...
                    << lowestPoint);
    const Hz_u carrierSpacing{1250000};
    Ptr<SpectrumValue> c = Create<SpectrumValue>(
    ...
    std::size_t numAllocatedSubcarriersPer20MHz = 13; // Scaled from 52 (52/4 = 13) for 1.25 MHz carrier spacing
    ...
    dBr_u lowestPoint)
{
    ...
    NS_ASSERT_MSG(c->GetSpectrumModel()->GetNumBands() ==
                  ...
                  "Unexpected number of bands " << c->GetSpectrumModel()->GetNumBands());
    auto num20MhzBands = Count20MHzSubchannels(channelWidth);
    std::size_t numAllocatedSubcarriersPer20MHz = 14; // Scaled from 56 (56/4 = 14) for 1.25 MHz carrier spacing
    const auto txPowerPerBand = (txPower / numAllocatedSubcarriersPer20MHz) / num20MhzBands;
    NS_LOG_DEBUG("Power per band " << txPowerPerBand << "W");
...
// Static helper to access the cache for statistics
static SpectrumPowerCache& GetBandPowerCache()
{
    static SpectrumPowerCache powerCache;
    return powerCache;
}
...
{
    // Get reference to static cache instance (optimized with hash map + LRU)
    auto& powerCache = GetBandPowerCache();

    // Try to get cached result first (timestamp removed from key for better hit rate)
    SpectrumPowerCache::CacheKey key{psd, segments};
    if (auto cachedPower = powerCache.Get(key))
    {
        return *cachedPower;
    }

    // Cache miss - calculate power
...
std::vector<Watt_u>
WifiSpectrumValueHelper::GetBandPowerWBatch(
    Ptr<SpectrumValue> psd,
    const std::vector<std::vector<WifiSpectrumBandIndices>>& bandsSegments)
{
    NS_LOG_FUNCTION(psd << bandsSegments.size());

    // Phase 3A: Batch processing to reduce overhead
    // Instead of 45 separate GetBandPowerW calls (each with hash + lookup),
    // we do one optimized pass computing all band powers

    std::vector<Watt_u> results;
    results.reserve(bandsSegments.size());

    // For small batch sizes, fall back to individual calls (simpler)
    if (bandsSegments.size() <= 2)
    {
        for (const auto& segments : bandsSegments)
        {
            results.push_back(GetBandPowerW(psd, segments));
        }
        return results;
    }

    // For larger batches, optimize with single pass (no cache overhead)
    // This is faster than caching because:
    // 1. Sequential memory access (CPU cache friendly)
    // 2. No hash computation overhead
    // 3. No hash map lookup overhead
    // 4. Most PSDs are unique per packet anyway (low cache hit rate)

    // Get bandwidth (all bands have same width)
    auto bandIt = psd->ConstBandsBegin();
    if (psd->GetSpectrumModel()->GetNumBands() > 0 && !bandsSegments.empty() &&
        !bandsSegments[0].empty())
    {
        bandIt = psd->ConstBandsBegin() + bandsSegments[0].front().first;
    }
    const auto bandWidth = (bandIt->fh - bandIt->fl);

    // Process each band with optimized sequential access
    for (const auto& segments : bandsSegments)
    {
        auto powerWattPerHertz{0.0};
    ...
    NS_LOG_DEBUG("Phase 3A: Computed " << results.size() << " band powers in optimized batch");
    return results;
}

void
WifiSpectrumValueHelper::LogBandPowerCacheStats()
{
    auto& cache = GetBandPowerCache();
    uint64_t total = cache.GetHits() + cache.GetMisses();

    NS_LOG_UNCOND("╔═══════════════════════════════════════════════════════════════╗");
    NS_LOG_UNCOND("║  GetBandPowerW Cache Statistics (Optimized Hash+LRU)        ║");
    NS_LOG_UNCOND("╠═══════════════════════════════════════════════════════════════╣");
    NS_LOG_UNCOND("║  Cache Hits:       " << std::setw(10) << cache.GetHits()
                  << "  (" << std::fixed << std::setprecision(2)
                  << cache.GetHitRate() * 100 << "%)                ║");
    NS_LOG_UNCOND("║  Cache Misses:     " << std::setw(10) << cache.GetMisses()
                  << "  (" << std::fixed << std::setprecision(2)
                  << (total > 0 ? (cache.GetMisses() * 100.0 / total) : 0) << "%)                ║");
    NS_LOG_UNCOND("║  Total Lookups:    " << std::setw(10) << total << "                              ║");
    NS_LOG_UNCOND("║  Evictions:        " << std::setw(10) << cache.GetEvictions() << "                              ║");
    NS_LOG_UNCOND("║  Current Size:     " << std::setw(10) << cache.GetSize() << " / 10000                    ║");
    NS_LOG_UNCOND("╠═══════════════════════════════════════════════════════════════╣");
    NS_LOG_UNCOND("║  Performance Impact:                                          ║");
    NS_LOG_UNCOND("║  - Avoided " << cache.GetHits() << " redundant band power calculations       ║");
    NS_LOG_UNCOND("║  - Using O(1) hash map (was O(log N) red-black tree)        ║");
    NS_LOG_UNCOND("║  - LRU eviction is O(1) (was O(N) scan)                     ║");
    NS_LOG_UNCOND("╚═══════════════════════════════════════════════════════════════╝");
}

void
WifiSpectrumValueHelper::ClearBandPowerCache()
{
    // Phase 3D: Clear the band power cache on channel switch
    auto& cache = GetBandPowerCache();
    cache.Clear();
    NS_LOG_DEBUG("Phase 3D: Band power cache cleared on channel switch");
}

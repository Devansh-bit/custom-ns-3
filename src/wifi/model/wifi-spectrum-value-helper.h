...
class WifiSpectrumValueHelper
{
  public:
    ...
    /**
     * Batch version: Calculate power for multiple bands in a single operation (Phase 3A).
     * Much more efficient than calling GetBandPowerW in a loop - reduces hash overhead
     * and improves cache locality.
     *
     * @param psd received Power Spectral Density in W/Hz
     * @param bandsSegments vector of band segment lists (one per band to query)
     *
     * @return vector of band powers (same order as input)
     */
    static std::vector<Watt_u> GetBandPowerWBatch(
        Ptr<SpectrumValue> psd,
        const std::vector<std::vector<WifiSpectrumBandIndices>>& bandsSegments);

    /**
     * Log cache statistics for GetBandPowerW performance analysis.
     * Call this at the end of simulations to see cache effectiveness.
     */
    static void LogBandPowerCacheStats();

    /**
     * Clear the band power cache (Phase 3D).
     * Called on channel switch to invalidate cached power calculations.
     */
    static void ClearBandPowerCache();
};
